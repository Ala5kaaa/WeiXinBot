...
class FileUtils {
    ...
    static recordMoyu(userName, isWin) {
        const fs = require('fs');
        // 读取已有的用户记录
        let userRecords = {};
        try {
            userRecords = JSON.parse(fs.readFileSync('moYu.json'));
        } catch (err) {
            // 如果文件不存在或读取失败，则认为是新用户
        }
        ...
        // 保存用户记录
        fs.writeFileSync('moYu.json', JSON.stringify(userRecords));
    ...
    static getMoyuRate() {
        ...
        for (const [userName, record] of recordsArray) {
            const winRate = record.totalWins / record.totalGames;
            result += `${userName}  总摸鱼量：${record.totalGames}\n`;
        }
