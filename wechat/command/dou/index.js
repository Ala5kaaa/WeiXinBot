...
const MoyubangCommand = require("./MoyubangCommand");
...
function registerDouCommand() {
  ...
  DouCommandHandler.registerCommand("摸鱼榜", new MoyubangCommand());
  DouCommandHandler.registerCommand("摸鱼", new MoyuCommand());
...
function getDouCommand(mentionSelf, contentTrim) {
  ...
  if (mentionSelf) {
    if (contentTrim.includes("胜率榜")) {
      commandName = "胜率榜";
    } else if (contentTrim.includes("摸鱼榜")) {
      commandName = "摸鱼榜";
